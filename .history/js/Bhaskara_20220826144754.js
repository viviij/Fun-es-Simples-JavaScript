...
calcBhaska.onclick = () => {
  ...
  if(valorDelta < 0) {
    ...
    }else if (calculoDeX > 0){
      calculoDeX += 0.05;
    }
    
    let calculoDeX2 = (-bhaskaVB.value-Math.sqrt(valorDelta))/(2*bhaskaVA.value)-0.05;
    if(calculoDeX2 < 0) {
      calculoDeX2 -= 0.05;     //Arredonda o valor da segunda casa decimal
    }else if (calculoDeX2 > 0){
      calculoDeX2 += 0.05;
    }
    
    resBhaska.value = [calculoDeX.toFixed(1)+"~", calculoDeX2.toFixed(1)+"~"];
  }
