...
calcBhaska.onclick = () => {
  ...
  if(valorDelta < 0) {
    ...
    const calculoDeX = (-bhaskaVB.value+Math.sqrt(valorDelta))/(2*bhaskaVA.value); //Calculo arrendo o o valor da segunda casa
    const calculoDeX2 = (-bhaskaVB.value-Math.sqrt(valorDelta))/(2*bhaskaVA.value);//decimal
    
    resBhaska.value = [calculoDeX.toFixed(2), calculoDeX2.toFixed(2)];
  }
