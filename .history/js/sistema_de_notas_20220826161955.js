...
calcNota.onclick = () => {
  ...
  for (let i in tabuadaDo5) {
    ...
    if (int(notaRec.value)+2 >= tabuadaDo5[i] && int(notaRec.value)-1 == tabuadaDo5[i] || int(notaRec.value)-2 == tabuadaDo5[i] ||int(notaRec.value)-3 == tabuadaDo5[i]) {
      arreNota.value = tabuadaDo5[i];
    }else {
      arreNota.value = notaRec.value
    }
  }
}