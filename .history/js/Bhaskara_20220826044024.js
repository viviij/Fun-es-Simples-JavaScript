...
cb.onclick = () => {
  ...
  //Função de bhaskara
  const valorDelta = (bvB.value*bvB.value) + bvA.value*(bvC.value);
  if(valorDelta < 0) {
    valorBhaskara.value = "Delta é negativo";
  }else{
    const calculoDeX = (-bvB.value+valorDelta)/(2*bvA.value);
    const calculoDeX2 = (-bvB.value-valorDelta)/(2*bvA.value);
    alert(calculoDeX2, calculoDeX)
    valorBhaskara.value = [calculoDeX, calculoDeX2];
  }
